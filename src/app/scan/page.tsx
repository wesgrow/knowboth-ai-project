...
export default function ScanPage() {
  ...
  const { addPoints, moveToPantry, user, setUser } = useAppStore();
          ...
          {step==="upload"&&(
            <div>
              ...
              <div onClick={()=>fileRef.current?.click()} onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();e.dataTransfer.files[0]&&handleFile(e.dataTransfer.files[0]);}}
                style={{border:`2px dashed ${file?"#FF9F0A":"var(--border)"}`,borderRadius:16,padding:"32px 20px",textAlign:"center",cursor:"pointer",background:"var(--surf)",marginBottom:12,boxShadow:"var(--shadow)"}}>
                {preview
                  ?<img src={preview} alt="" style={{maxHeight:200,borderRadius:10,objectFit:"contain",margin:"0 auto"}}/>
                  :<><div style={{fontSize:48,marginBottom:10}}>🧾</div><div style={{fontSize:15,fontWeight:600,color:"var(--text)",marginBottom:4}}>Upload Your Bill</div><div style={{fontSize:13,color:"var(--text3)"}}>JPG · PNG · PDF · Any store · Any language</div></>
                }
              </div>
              {file&&(
