...
export default function PantryPage() {
  const {
    pantry, updatePantryQty,
    removeFromPantry, restockItem, user,
  } = useAppStore();
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "16px 14px" }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: "var(--text-muted)" }}>
              {pantry.length} items · {low} low stock
            </p>
          </div>
          <div style={{
        ...
        <div style={{
          ...
          {[
            { l: "Items", v: pantry.length, c: "var(--gold)" },
        ...
        {pantry.length === 0 && (
          <div style={{ textAlign: "center", padding: "60px 0" }}>
            ...
            <p style={{ fontSize: 12, color: "var(--text-muted)" }}>
              Upload bills to auto-fill your pantry
            </p>
          </div>
        )}
          ...
          {pantry.map(item => (
            <div key={item.id} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <button
                    onClick={() => updatePantryQty(item.id, item.qty - 1)}
                    style={{
                  ...
                  <span style={{
                    fontSize: 16, fontWeight: 900, color: "var(--teal)",
                ...
                <button
                  onClick={() => { setRestock(item); setRqty(1); }}
                  style={{
      ...
      {restock && (
        <div
          onClick={e => e.target === e.currentTarget && setRestock(null)}
          style={{
        ...
        >
          <div style={{
            ...
            <div style={{
              ...
              <button
                onClick={() => setRqty(q => Math.max(1, q - 1))}
                style={{
              ...
              <span style={{
                ...
                fontSize: 28, fontWeight: 900, color: "var(--teal)",
              }}>{rqty}</span>
              <button
                onClick={() => setRqty(q => q + 1)}
                style={{
                  width: 36, height: 36, borderRadius: 8,
                  border: "1px solid var(--border)", background: "var(--surf)",
                  color: "var(--text)", cursor: "pointer", fontSize: 18,
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => setRestock(null)} className="btn-ghost"
                style={{ padding: "12px 20px" }}>Cancel</button>
              <button
                onClick={() => {
                  for (let i = 0; i < rqty; i++) restockItem(restock);
                  toast.success(`🛒 ${restock.name} → Cart`);
                  setRestock(null);
                }}
                className="btn-gold" style={{ flex: 1, padding: 12 }}>
                Add to Cart
              </button>
            </div>
          </div>
