...
function CompareContent() {
  ...
  async function doCompare(term?: string) {
    ...
    const { data: brands } = await supabase
      .from("brands").select("id, name, slug").in("id", brandIds as string[]);

    const { data: locs } = locIds.length > 0
      ? await supabase.from("store_locations").select("id, branch_name, city, zip").in("id", locIds as string[])
      : { data: [] };
    ...
    (locs || []).forEach((l: any) => { locMap[l.id] = l; });
