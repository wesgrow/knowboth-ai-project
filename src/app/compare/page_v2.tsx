...
function CompareContent() {
  ...
  useEffect(() => {
    if (params.get("item")) doCompare(params.get("item")!);
  }, []);
  ...
  async function doCompare(term?: string) {
    ...
    const dealIds = dealItems.map(d => d.deal_id);
    ...
    const approvedDealIds = new Set((deals || []).map(d => d.id));
    const filtered = dealItems.filter(i => approvedDealIds.has(i.deal_id));

    const brandIds = [...new Set((deals || []).map(d => d.brand_id).filter(Boolean))];
    const locIds = [...new Set((deals || []).map(d => d.location_id).filter(Boolean))];

    const { data: brands } = await supabase
      .from("brands").select("id, name, slug").in("id", brandIds);

    const { data: locs } = locIds.length > 0
      ? await supabase.from("store_locations").select("id, branch_name, city, zip").in("id", locIds)
      : { data: [] };
    ...
    const merged = filtered.map(item => {
      const deal = dealMap[item.deal_id];
      return {
        ...item,
        deal,
        brand: brandMap[deal?.brand_id],
        location: locMap[deal?.location_id],
      };
    });
      ...
      <div style={{ maxWidth: 800, margin: "0 auto", padding: "16px 14px" }}>
        <div style={{ marginBottom: 20 }}>
          ...
          <p style={{ fontSize: 12, color: "var(--text-muted)" }}>
            Find the cheapest store · See price source
          </p>
        </div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
          <input className="input" value={q} onChange={e => setQ(e.target.value)}
            ...
            placeholder="Search toor dal, rice, ghee..." style={{ flex: 1 }} />
          <button className="btn-gold" onClick={() => doCompare()}
        ...
        {!loading && results.length === 0 && q && (
          <div style={{ textAlign: "center", padding: "60px 0" }}>
            ...
            <p style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 6 }}>
              Try searching a different item name
            </p>
          </div>
        )}
        ...
        {results.length > 0 && (
          <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 12 }}>
            {results.length} stores · sorted cheapest first
          </div>
        )}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {results.map((item, i) => {
            ...
            const daysLeft = item.deal?.sale_end
              ? Math.ceil((new Date(item.deal.sale_end).getTime() - Date.now()) / 86400000)
              : null;
            ...
            return (
              <div key={item.id} style={{
                ...
                boxShadow: i === 0 ? "0 0 20px rgba(245,166,35,0.08)" : "none",
              }}>
                {i === 0 && (
                  <div style={{
                ...
                <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
                  ...
                  <div style={{
                    ...
                    background: i === 0 ? "rgba(245,166,35,0.12)" : "var(--surf2)",
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {item.location && (
                      <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 1 }}>
                        {item.location.branch_name} · {item.location.city}
                      </div>
                    )}
                    ...
                    <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginTop: 6 }}>
                      <span className={`pill fresh-${fresh.level}`} style={{ fontSize: 9 }}>
                        {fresh.label}
                      </span>
                      {daysLeft !== null && daysLeft >= 0 && (
                        <span style={{
                          borderRadius: 20, padding: "2px 8px", fontSize: 9, fontWeight: 700,
                          background: daysLeft <= 2 ? "rgba(255,71,87,0.1)" : "rgba(245,166,35,0.1)",
                          color: daysLeft <= 2 ? "var(--red)" : "var(--gold)",
                          border: `1px solid ${daysLeft <= 2 ? "rgba(255,71,87,0.3)" : "rgba(245,166,35,0.3)"}`,
                        }}>
                ...
                <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 10 }}>
                  <button onClick={() => handleAdd(item)}
                    className={inCart ? "btn-ghost" : "btn-gold"}
