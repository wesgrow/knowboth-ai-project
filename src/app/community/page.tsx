...
const STORE_COLORS: Record<string, string> = {
  "patel-brothers": "#4caf72",
  "india-bazaar": "#9b6fe8",
  "apna-bazar": "#e08918",
  "swadesh-grocery": "#5b9dee",
  "india-grocery-spices": "#e05c6e",
  "india-cash-carry": "#e05c6e",
};
...
export default function CommunityPage() {
  ...
  async function fetchDeals() {
    ...
    const merged: Deal[] = dealRows.map((d: any) => ({
      id: d.id,
      brand: bMap[d.brand_id] || { name: "Unknown Store", slug: "" },
      location: locMap[d.location_id],
      posted_by: d.posted_by,
      flyer_image_url: d.flyer_image_url,
  ...
  async function toggleLike(dealId: string, liked: boolean) {
    ...
    if (liked) {
      await supabase.from("deal_likes").delete().eq("deal_id", dealId).eq("user_id", currentUserId);
    } else {
      await supabase.from("deal_likes").insert({ deal_id: dealId, user_id: currentUserId });
    }
  ...
  function share(deal: Deal) {
    ...
    if (typeof navigator !== "undefined" && typeof navigator.share === "function") {
      navigator.share({ title: "Deal Alert!", text });
    } else {
